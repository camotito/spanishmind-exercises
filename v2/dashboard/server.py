...
externas -- http.server de la stdlib). Permite:
  - listar/editar las oraciones (texto_enunciado) de los items de v2/content/<tema>.json
  - ocultar/mostrar un bloque de ejercicios sin borrarlo (campo "oculto")
  - re-renderizar v2/out/<tema>.html automaticamente al guardar
  - lanzar la sincronizacion de ejercicios_permitidos (index.yaml) en un click
  - servir v2/libro_pipeline/indice_unidades.html embebido (indice buscable del libro)

Uso:
    python3 v2/dashboard/server.py           # arranca en http://127.0.0.1:8765
    python3 v2/dashboard/server.py --port 9000
...
OUT_DIR = os.path.join(V2_DIR, "out")
INDEX_YAML = os.path.join(V2_DIR, "index.yaml")

sys.path.insert(0, V2_DIR)
import render  # noqa: E402  (necesita el sys.path.insert de arriba)

sys.path.insert(0, os.path.join(V2_DIR, "libro_pipeline"))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/" or path == "":
            self._static("v2/dashboard/index.html")
        elif path == "/api/temas":
