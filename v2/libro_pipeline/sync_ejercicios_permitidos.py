#!/usr/bin/env python3
"""Deriva `ejercicios_permitidos` en v2/index.yaml a partir de las tipologias
REALES encontradas en v2/ejemplos/<tema>.json, para cada tema con unidad_libro.
...
def main():
    ...
    for t in indice:
        ...
        if tipos is None:
            print(f"⚠️  {tema}: no existe v2/ejemplos/{tema}.json, se salta")
            continue
        if tipos != t.get("ejercicios_permitidos"):
            cambios.append((tema, t.get("ejercicios_permitidos"), tipos))

    if not cambios:
        print("✅ ejercicios_permitidos ya esta sincronizado con v2/ejemplos/ para todas las unidades con unidad_libro.")
        return
